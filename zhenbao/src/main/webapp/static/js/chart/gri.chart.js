...
//		}
		...
		if (mainYAxis.dataFormat == 4 || mainYAxis.dataFormat == 3){
			//转成时分秒格式H:mm:ss或百分比
			mainYAxis.labels.formatter = function(){
		...
		if (cOptions.chart.type != 'pie'){
            cOptions.tooltip.formatter = function(){
				...
				var s = '<div style="padding:0px;"><b>' + xName + yName + '</b></div><table style="width: 150px">';                
				$.each(this.points, function(i, point) {
                    var dataFormat = cOptions.series[point.series.index].dataFormat || mainYAxis.dataFormat;
					var value = formatValue(dataFormat, point.y);
					...
					s += '<tr><td style="padding: 0px 2px" >' + title + ' </td>' 
...
//                    symbolWidth: 12 
                ...
                credits: {
                    enabled: false,
                    href: 'http://www.iflytek.com',
                    text: 'www.iflytek.com',
                    position: {
                        align: 'right',
                        x: -10,
